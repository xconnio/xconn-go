...
func dialQUIC(t *testing.T, address string, spec xconn.SerializerSpec) *xconn.QUICSession {
	...
	session, err := xconn.ConnectQUIC(context.Background(), address, realmName, &xconn.QUICDialerConfig{
...
func TestQUICMultiplexedStream(t *testing.T) {
	...
	conn := <-listener.AcceptSession()
	require.NotNil(t, conn.QUICConn)

	serverStream, err := conn.OpenStream()
	require.NoError(t, err)
