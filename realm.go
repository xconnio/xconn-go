...
func uriAndIDIfNeedsAuth(msg messages.Message) (uri string, requestID int64, needsAuth bool) {
	switch m := msg.(type) {
	case *messages.Call:
...
func (r *Realm) ReceiveMessage(baseSession BaseSession, msg messages.Message) error {
	...
	if needsAuth && !r.isAuthorized(baseSession.AuthRole(), msg.Type(), uri) {
		errMsg := messages.NewError(int64(msg.Type()), requestID, map[string]any{},
