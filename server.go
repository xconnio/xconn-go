...
func (s *Server) ListenAndServeQUIC(address string, tlsConfig *tls.Config) (*QUICListener, error) {
	...
	l := &QUICListener{
		...
		server:   s,
		conns:    make(chan *QUICPeerSession, 8),
...
// AcceptSession returns a channel that receives an event each time an authenticated QUIC client connects.
func (l *QUICListener) AcceptSession() <-chan *QUICPeerSession {
	return l.conns
}
